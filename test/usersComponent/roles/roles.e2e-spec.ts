...
describe('Roles (e2e)', () => {
  ...
  describe('/api/roles (PUT)', () => {
    ...
    it('should return status FORBIDDEN because it a Unknown', async () => {
      await request(app.getHttpServer()).put('/api/roles').send(role).expect(HttpStatus.FORBIDDEN)
    })
  ...
  describe('/api/roles (GET)', () => {
    ...
    it('should return status FORBIDDEN because it a Unknown', async () => {
      await request(app.getHttpServer()).get('/api/roles').expect(HttpStatus.FORBIDDEN)
    })
  })
